/* Установка на терминал лицензии ППУ. (c) gsr 2011, 2019 */
...
static struct lprn_license_info licenses[MAX_LPRN_LICENSES];
static int nr_licenses;

/* Чтение файла лицензий */
static int read_license_file(const char *name, struct lprn_license_info *licenses)
{
	struct stat st;
	div_t d;
	int fd, ret = -1;
	if (stat(name, &st) == -1)
		...
		d = div(st.st_size, sizeof(struct lprn_license_info));
		if ((d.quot == 0) || (d.rem != 0))
			fprintf(stderr, "Файл %s имеет неверный размер.\n", name);
		else if (d.quot > MAX_LPRN_LICENSES)
...
/*
 * Поиск лицензии ППУ по хешу заводского номера. Если хеш заводского
 ...
 * лицензии.
 */
static const struct md5_hash *find_license(const struct lprn_license_info *licenses,
		int nr_licenses, const struct md5_hash *number)
{
	...
	if (n == 1)
		...
		write_lic_signature(LPRN_LIC_SIGNATURE_OFFSET, LPRN_LIC_SIGNATURE);
	return ret;
}

/* Вывод лицензии ППУ в stdout */
...
int main(int argc, char **argv)
{
	...
	if (argc != 2)
		fprintf(stderr, "Укажите имя файла списка лицензий для работы с ППУ.\n");
	else if (!read_term_number(TERM_NUMBER_FILE, &number))
		fprintf(stderr, "Не найден модуль безопасности.\n");
	else if (check_lic_signature(LPRN_LIC_SIGNATURE_OFFSET, LPRN_LIC_SIGNATURE))
			fprintf(stderr, "Установка лицензии для работы с ППУ на данный "
				"терминал невозможна;\n"
		...
		if (nr_licenses > 0){
			...
			if (license == NULL)
				fprintf(stderr, "Не найдена лицензия ППУ для этого терминала.\n");
			else{
