/* Создание файла списка лицензий ППУ. (c) gsr 2011, 2019 */
...
static void make_license_info(const uint8_t *number, struct lprn_license_info *li)
{
...
/* Создание хеша лицензии ТКИ */
	...
	for (i = 0; i < TERM_NUMBER_LEN; i++)
		buf[sizeof(buf) - i - 1] = swap_nibbles(~buf[i]);
...
static int make_licenses(int fd, struct lprn_license_info *licenses)
{
	int n = 0, ret;
	uint8_t number[TERM_NUMBER_LEN];
	while ((ret = read_number(fd, number)) == 1){
		if (n == MAX_LPRN_LICENSES){
...
static void write_licenses(const struct lprn_license_info *licenses, size_t nr_licenses)
{
	write(STDOUT_FILENO, licenses, nr_licenses * sizeof(struct lprn_license_info));
}
