...
int cheque_reissue_docs_draw() {
	...
	draw_title(screen, fnt, "Снятие у документа отметки переоформления");

	SetTextColor(screen, clBlack);

	sprintf(text, docs.count > 0 ? "Выберите документ из списка и нажмите \"-\"" :
		"Нет документов с переоформлением в корзине ФП");
...
static void del_item() {
	...
	sprintf(text, "У документа с номером %.14lld будет снята отметка о переоформлении.\nПродолжить?", n);
