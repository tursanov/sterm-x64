...
static struct bank_info *bank_info_clone(const struct bank_info *v)
{
	if (v == NULL)
		return NULL;

	struct bank_info *result = (struct bank_info *)malloc(sizeof(struct bank_info));
	*result = *v;
	return result;
}
		        ...
		        if (d->k->y != NULL)
		        {
    		        ...
    		        if (d->k->y->op != '-')
		            {
		                ...
		                else
		                {
		                    ...
		                    {
		                        d->name = strdup("ГАШЕНИЕ ВОЗВРАТА");
		                    }
		                    else
		                    {
		                        d->name = strdup("ОФОРМЛЕНИЕ");
		                    }
		                }
		            }
		            else
